  ...
  def start_server(self, int_counter): 
    ...
    while(int_counter > 0): 
      ...
      if not check_if_file_exists(curr_env_file):
        print("Failed to find environment file", flush=True)
        consecutive_fails += 1
        if consecutive_fails > 120:
          raise FileNotFoundError(f"Environment file not found at {curr_env_file}")
        time.sleep(1.0)
        continue
      else:
        consecutive_fails = 0
      ...
      # input to our personas. So we first retrieve it.
      # Try and save block for robustness of the while loop.
      #TODO: Maybe alter to pass if loading fails. idk what sort of consistency is in the loop
      try:
        with open(curr_env_file) as json_file:
          new_env = json.load(json_file)
          #env_retrieved = True
        ...
        print("Failed to decode environment file", flush=True)
        consecutive_fails += 1
        if consecutive_fails > 120:
          raise e
